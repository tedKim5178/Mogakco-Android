...
class CompactCalendarController {
    ...
    private gdghackathon.mogakco.tools.CompactCalendarView.CompactCalendarViewListener listener;
    ...
    void setListener(gdghackathon.mogakco.tools.CompactCalendarView.CompactCalendarViewListener listener) {
    ...
    void onMeasure(int width, int height, int paddingRight, int paddingLeft) {
        ...
        bigCircleIndicatorRadius = shouldDrawIndicatorsBelowSelectedDays && eventIndicatorStyle == CompactCalendarView.SMALL_INDICATOR ? bigCircleIndicatorRadius * 0.5f : bigCircleIndicatorRadius * 0.7f;
    }
    ...
    void drawEvents(Canvas canvas, Calendar currentMonthToDrawCalender, int offset) {
        ...
        if (uniqEvents != null) {
            for (int i = 0; i < uniqEvents.size(); i++) {
                ...
                float yPosition = weekNumberForMonth * heightPerDay + paddingHeight + 34;
        ...
        for (int dayColumn = 0, dayRow = 0; dayColumn <= 6; dayRow++) {
            ...
            if (dayRow == 0) {
                ...
                if (!drawLine) {
                    ...
                    drawLine = true;
                    canvas.drawLine(35, yPosition + paddingHeight, (widthPerDay - paddingWidth) * 13 + 15, yPosition + paddingHeight, paint);
                }
